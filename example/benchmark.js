...
function runBackendBenchmark(next) {
	...
	async.whilst(
	    ...
	    function (callback) {
	        request({url: 'http://localhost:5001/backend.html', headers:{'Accept':'text/html'}}, function (error, response, body) {
...
function runProxyBenchmark(next) {
	...
	async.whilst(
	    ...
	    function (callback) {
	        count++;
	        request({url: 'http://localhost:5000/backend.html', headers:{'Accept':'text/html'}}, function (error, response, body) {
			  if (!error && response.statusCode == 200) {
			    callback();
			  }
			});
