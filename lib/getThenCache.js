    ...
    if(!options.explicitNoCache && options.cacheTTL > 0) {
        ...
        cache.get(options.cacheKey, function(err, content, oldContent) {
            ...
            if (content) {
                ...
                //logger.info('CACHE ' + options.cacheKey,{tracer:options.tracer, responseTime: timing, pcType:options.type});
                //logger.debug('Cache HIT for key: ' + options.cacheKey,{tracer:options.tracer,pcType:options.type});
                //logger.statsd.increment('PC', options.statsdKey + '.cacheHit');
                stream.end(content);
                return;
            } else {
                //logger.debug('Cache MISS for key: ' + options.cacheKey,{tracer:options.tracer,pcType:options.type});
                //logger.statsd.increment('PC', options.statsdKey + '.cacheMiss');
            }
            ...
            pipeAndCacheContent(function(err, content) {
                ...
                cache.set(options.cacheKey, content, options.cacheTTL, function(err) {
                    //logger.debug('Cache SET for key: ' + options.cacheKey + ' @ TTL: ' + options.cacheTTL,{tracer:options.tracer,pcType:options.type});
                });
            });
        });
    ...
    function pipeAndCacheContent(next) {
        ...
        var r = request({url: options.url, agent: false, timeout: options.timeout, headers: options.headers})
            ...
            .on('end', function() {
                ...
                next(null, content);
                var timing = (new Date() - start);
                //logger.info('URL ' + options.url,{tracer:options.tracer, responseTime: timing, pcType:options.type});
                //logger.statsd.timing('PC', options.statsdKey + '.responseTime',timing);
            });
