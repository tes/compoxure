...
var errorTemplate = "<div style='color: orange; font-weight: bold; font-family: monospace;'>Error: <%= err %></div>";
                    ...
                    if (err.statusCode === 404) {
                        ...
                        if(!req.backend.quietFailure) {
                            ...
                            if(oldContent) {
                                ...
                                //logger.error(errorMsg(options), {tracer:req.tracer});
                        ...
                        //logger.statsd.increment('PC', options.statsdKey + '.error');
                        var elapsed = (new Date() - req.timerStart), timing = (new Date() - start);
                        //logger.error('%s FAILED in %s, elapsed %s.', options.url, timing, elapsed, {tracer:req.tracer});
