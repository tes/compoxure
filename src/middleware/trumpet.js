...
TrumpetProxy.prototype.middleware = function(req, res, next) {
	...
	if (self.transformations) {
		...
		tr.on('end', function () {
			...
			self.eventHandler.logger('info', "Page composer response completed", {tracer: req.tracer,responseTime: responseTime, dataTime: dataTime});
			self.eventHandler.stats('timing','responseTime',responseTime);
		});

		// Note that we don't pipe the response directly from trumpet so that we can deal with 404s
